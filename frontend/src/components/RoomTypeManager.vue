...
const formData = ref<RoomTypeForm>({
  name: '',
  description: '',
  baseRate: 99.99,
  maxOccupancy: 1,
...
const addRoomType = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      error.value = data.error || 'Failed to add room type'
    }
  } catch (e) {
    error.value = 'Failed to add room type'
  } finally {
    loading.value = false
  }
}
...
const updateRoomType = async () => {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/room-types/${editingRoomType.value.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: formData.value.name,
    ...
    if (response.ok) {
      ...
      error.value = data.error || 'Failed to update room type'
    }
  } catch (e) {
...
const deleteRoomType = async (roomTypeId: string) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const data = await response.json()
      error.value = data.error || 'Failed to delete room type'
    }
  } catch (e) {
    error.value = 'Failed to delete room type'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="room-type-manager">
    ...
    <div class="form-container">
      ...
      <form @submit.prevent="editingRoomType ? updateRoomType() : addRoomType()">
        <div class="form-row">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input
              id="baseRate"
              v-model.number="formData.baseRate"
              type="number"
              step="0.01"
              min="0"
              placeholder="99.99"
        ...
        <div class="form-group">
          ...
          <input
            id="amenities"
            v-model="formData.amenitiesInput"
            type="text"
            placeholder="WiFi, TV, Mini Bar, Coffee Maker"
            :disabled="loading"
          />
        </div>
            ...
            <tr v-for="roomType in roomTypes" :key="roomType.id">
              ...
              <td>
                <div v-if="roomType.amenities && roomType.amenities.length" class="amenities">
                  <span v-for="amenity in roomType.amenities" :key="amenity" class="badge badge-amenity">
                    {{ amenity }}
                  </span>
                </div>
                <span v-else>-</span>
              ...
              <td>
                <span class="badge" :class="getStatusBadgeClass(roomType.status)">
                  {{ roomType.status }}
                </span>
              </td>
              <td class="actions">
