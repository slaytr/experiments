...
const activeRoomTypes = computed(() => {
  return roomTypes.value.filter(rt => rt.status === 'active')
})
...
const filteredRooms = computed(() => {
  ...
  if (roomTypeFilter.value !== 'all') {
    filtered = filtered.filter(r => r.roomTypeId === roomTypeFilter.value)
  }
...
const addRoom = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      error.value = data.error || 'Failed to add room'
    }
  } catch (e) {
...
const updateRoom = async () => {
  ...
  try {
    const response = await fetch(`${API_URL}/rooms/${editingRoom.value.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        roomNumber: formData.value.roomNumber,
    ...
    if (response.ok) {
      ...
      error.value = data.error || 'Failed to update room'
    }
  } catch (e) {
...
const deleteRoom = async (roomId: string) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const data = await response.json()
      error.value = data.error || 'Failed to delete room'
    }
  } catch (e) {
...
<template>
  <div class="room-manager">
    ...
    <div class="form-container">
      ...
      <form @submit.prevent="editingRoom ? updateRoom() : addRoom()">
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <select id="roomTypeId" v-model="formData.roomTypeId" :disabled="loading" required>
              ...
              <option v-for="roomType in activeRoomTypes" :key="roomType.id" :value="roomType.id">
                {{ roomType.name }}
              </option>
            </select>
          </div>
